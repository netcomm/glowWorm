...
public final class CodedOutputStream{
    ...
    public void writeGroup(int fieldNumber, int groupItemCntParm){
        //writeTag(fieldNumber, WireFormat.WIRETYPE_START_GROUP);
        writeRawVarint32(groupItemCntParm);
    }
    ...
    public void writeRawBytes(byte[] value, int offset, int length){
    	...
    	//bos.append(value, offset, length);
    }

    public void writeRawBytes(Buffer data) throws IOException {
    	bos.write(data.data, data.offset, data.length);
    	//bos.append(data.data, data.offset, data.length);
    }
    ...
    public void writeDouble(double dvParm)
    {
        long v = Double.doubleToLongBits(dvParm);
        byte[] tmpBytes = getBytesFromLong(v);
        writeRawBytes(tmpBytes, 0, 8);
    }
    
    /*public void writeDouble(double dvParm)
    {
    	byte[] tmpBytes = Double.toString(dvParm).getBytes();
    	writeInt32(tmpBytes.length);
    	writeRawBytes(tmpBytes, 0, tmpBytes.length);
    }*/
